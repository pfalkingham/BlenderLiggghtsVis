import bpy
from bpy.props import StringProperty, IntProperty, FloatVectorProperty, CollectionProperty, BoolProperty, IntVectorProperty
import os
import numpy as np
...
class ParticleDataset(bpy.types.PropertyGroup):
    ...
    use_optimized_parser: BoolProperty(name="Use Optimized Parser", default=True,
                                     description="Use NumPy-based parser for faster file loading")
    use_mesh_update: BoolProperty(name="Update Mesh", default=True,
                                description="Update existing vertices when possible instead of recreating the mesh")
    use_parallel_processing: BoolProperty(name="Parallel Processing", default=False,
                                        description="Use parallel processing to load files in background")
    last_vertex_count: IntProperty(default=0)
    
    # Make reference dictionary a class variable to avoid issues with reinitialization
    _ref_positions_dict = {}
    
    is_sorted: BoolProperty(name="Data Sorted", default=False, description="Indicates if the particle data is pre-sorted by ID")
